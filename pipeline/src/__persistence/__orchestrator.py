# --- TypeHinting ---
from .strategies.serialization.abstracts import AbstractSerializer, AtomicSerializer
# --- Libraries ---
...
class PersistenceOrchestrator:
    ...
    def _execute_atomic_save(self, target: str, data: Any, plan: ExecutionPlan):
        """
        Standard save for whole objects. 
        Flow: Data -> Serializer -> Integrity -> Adapter.
        """
        # 1. Serialize the whole object into a payload (str or bytes)
        payload = plan.serializer.encode(data)
        
        # 2. Calculate integrity if a strategy exists
        checksum = None
        if plan.integrity:
            # We assume AtomicIntegrity.calculate(payload) here
            checksum = plan.integrity.calculate(
                payload.encode() if isinstance(payload, str) else payload
            )

        # 3. Write via the Adapter
        self._adapter.write(target, payload, mode=plan.target_mode)
        
        # 4. Return the checksum or a success indicator
